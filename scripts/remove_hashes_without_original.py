...
for folder in sys.argv[1:]:
    ...
    for file in (bar := tqdm(sorted(mylist))):
        ...
        if file.suffix.lower() == ".hash":
            ...
            if (
                original_file not in mylist and not original_file.exists()
            ):  # First check against the list in memory. It'll be faster than going to the filesystem every time.
                print(file)
                # file.unlink()
            continue
